...
#
# Соответствующая ограничениям функция задана в виде векторной ФНП g:
# R^2 -> R^m,
...
def SQP_step(x: float, y: float, Lambda: np.array,
             ...
             dL: Callable[[float, float, np.array], np.array]) \
        -> tuple[float, float, np.array]:
    '''
    Делает шаг алгоритма в заданной точке, на выходе получая шаги в сторону
    уменьшения градиента функции Лагранжа по координатам и по множителя
    Лагранжа
...
def SQP_method(x_0: float, y_0: float, Lambda_0: np.array,
               ...
               Hg: Callable[[float, float], np.array],
               Jg: Callable[[float, float], np.array],
               dL: Callable[[float, float, np.array],
                            tuple[float, float, np.array]],
               goal: float = 1e-5,
               lr: float = 1,
    ...
    Lambda - вектор размерностью (m, 1) из начальных значений множителей
    Лагранжа
    f - целевая функция
    Hf - функция, вычисляющая матрицу Гессе целевой функции в точке
    Hg - функция, вычисляющая матрицу Гессе ограничивающей функции в точке
    Jg - функция, вычисляющая матрицу Якоби ограничивающей функции в точке
    dL - функция, вычисляющая градиент функции Лагранжа в точке
    goal - значение нормы градиента функции Лагранжа, при котором поиск
    останавливается
    ...
    if show:
        print(
            f"Шаг 0:\t x = {x},\t y = {y},\t f = {f(x, y)},\t |grad L| = \
            {np.sqrt(np.sum(dL(x, y, Lambda) ** 2))}")
    ...
    while abs(grad_L) > goal:
        ...
        if show:
            print(
                f"Шаг {i + 1}:\t x = {x:.5f},\t y = {y:.5f},\t f = \
                    {f(x, y):.4f},\t |grad L| = {grad_L:.5e}")
